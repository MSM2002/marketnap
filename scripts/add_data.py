...
def load_csv(csv_path: Path) -> pl.DataFrame:
    # Disable inference so dates come in as strings
...
def append_or_overwrite(
    ...
    raw = load_csv(csv)
    df = align_and_cast(raw, SCHEMA, date_format=date_format)
...
def main() -> None:
    ...
    parser.add_argument(
        "--date-format",
        default="%d-%m-%Y",
        help="Date format for parsing date columns (default: dd-mm-yyyy)",
    )
